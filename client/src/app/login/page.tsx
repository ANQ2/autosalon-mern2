...
import {useAuthStore, User} from "src/store/auth";
...
function getGqlErrorMessage(e: unknown): string {
    // Пытаемся извлечь сообщение из объектов, похожих на ApolloError
    if (typeof e === "object" && e !== null) {
        ...
        const msg =
            maybe.graphQLErrors?.[0]?.message ??
            maybe.networkError?.message ??
            maybe.message ??
            "Ошибка входа";
...
export default function LoginPage() {
    ...
    const [login, { loading }] = useMutation<LoginMutationData, LoginMutationVars>(LOGIN);
    ...
    return (
        <div className="mx-auto max-w-md space-y-4">
            <div className="space-y-1">
                ...
                <p className="text-sm text-neutral-400">Войдите, чтобы открыть каталог, чаты и CRM по роли.</p>
            </div>
